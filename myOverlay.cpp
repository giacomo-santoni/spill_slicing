...
void myOverlay(std::string outFileName,
               ...
               double spillPOT = 7.5E13)
{
  ...
  for (int spillId = 0;; ++spillId)
  {
    ...
    std::generate(times.begin(),
                  ...
                  []()
                  { return getInteractionTime_LBNF(); });
    ...
    for (const auto &eventTime : times)
    {
      nuIntEdepTree->GetEntry(evtIt);
      nuIntGRtrkTree->GetEntry(evtIt);

      outGRtrk.CopyFrom(nuIntGRtrk);
      outGRtrk.EvtVtx[3] = eventTime;
      ...
      for (auto &t : event->Trajectories)
      {
        ...
        for (auto &p : t.Points)
        {
          p.Position.SetT(eventTime + p.Position.T());
        }
      ...
      for (auto &d : event->SegmentDetectors)
      {
        for (auto &h : d.second)
        {
          h.Start.SetT(eventTime + h.Start.T());
          h.Stop.SetT(eventTime + h.Stop.T());
